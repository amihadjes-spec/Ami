...
1. Query Gmail with `label:Ami/Event-Pending` only (positive filter, no negation), then for each returned thread inspect its `labelIds` directly in the response and skip any thread that already has `Ami/Event-Notified-WhatsApp`'s label ID — the second label is the idempotency guard that stops the same thread being proposed again on a later run.

   > **Known Issue: Broken Label Negation (confirmed 2026-07-23, same underlying Gmail-tool bug documented in [`agents/email-event-calendar-agent.md`](email-event-calendar-agent.md)).** `-label:X` in a search query is not reliable in this Gmail tool — it can return threads that already carry the label being negated. Do not rely on `-label:Ami/Event-Notified-WhatsApp` in the query itself. Query on `label:Ami/Event-Pending` alone, then filter out already-notified threads by checking `labelIds` on each result in code/logic, exactly as the email agent's spec does for its own equivalent query.
...
* **`run_summary` is never skipped, even when a `proposal` also went out this run**: the two channels are independent and both required on every run — sending a `proposal` is not a substitute for the `run_summary` push. See "Critical fix — run_summary silently skipped when a proposal was also sent" below for the incident that made this explicit.
...
## Critical fix — run_summary silently skipped when a proposal was also sent (2026-07-26)

**Trigger**: user reported not receiving the usual ntfy run summary push after a scheduled run, despite receiving the WhatsApp proposal message fine. Investigated against the real evidence rather than assuming: Task Scheduler showed `LastTaskResult: 0`, the run's log entry (`043e4cd`) showed a clean, fully-completed summary with a new commit and push, and `docker logs waha` confirmed `POST /api/sendText` returned `201` at the time the proposal reached the phone — so the run itself was healthy end to end on every previously-checked dimension.

Comparing this run's log text against three prior runs (`b2ca101`, `3ae9fd7`, and a 21:15 run) made the pattern visible: every one of those three runs — none of which produced a new proposal — explicitly stated "נשלח סיכום ריצה ב-ntfy" in its summary. The run that *did* send a proposal (`043e4cd`) contained no such line at all. The one variable that changed was the presence of a `proposal` in that run.

**Root cause**: nothing in this document ever stated that sending a `proposal` and sending the `run_summary` are independent, both-required actions. A stateless session reconstructing behavior from "Two delivery channels, by `kind`" alone could read "proposal → WhatsApp, everything else → ntfy" as "if a proposal went out, that's this run's delivery" and treat `run_summary` as redundant — which is what appears to have happened here, silently and without a logged error.

**Fix**: added an explicit rule directly after "Two delivery channels" (§2) stating `run_summary` must go out via ntfy on every run regardless of whether a `proposal` was also sent this run. This is not a bug in `ntfy-notify.mjs` itself (the script has no conditional logic around this — see its source) or in WAHA/ntfy.sh connectivity (confirmed reachable); it was a gap in the spec's wording that a future run could reproduce again if not written down explicitly, same failure shape as "Critical fix — last-reply-wins restored" above.
