...
**הפתרון בפועל: קישור חיפוש לפי נושא**, לא קישור ישיר להודעה:
`https://mail.google.com/mail/u/0/#search/subject%3A%22<subject-url-encoded>%22`
(רווחים → `+`, מרכאות → `%22`, נקודתיים → `%3A`). זה תמיד עובד כי זו שאילתת
חיפוש טקסטואלית רגילה של Gmail, לא תלוי בסכמת מזהים פנימית כלשהי — אבל הוא
פותח תוצאות חיפוש (לרוב שורה אחת, המייל הרלוונטי) ולא קופץ ישירות לתוך ההודעה
עצמה. אם לנושא המייל יש תווים מיוחדים בעברית/אימוג'ים וכו', יש לקודד אותם
כראוי ב-URL לפני השימוש.
