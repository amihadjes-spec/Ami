...
3. **יצירת האירוע** — במקרה של אישור, האג'נט יוצר את האירוע ביומן Google Calendar
   ...
   אם צוינו). **סדר הפעולות מחייב** (ראו "קישור למייל המקור" בסוף הסעיף):
   (א) יצירת האירוע עם `description` שמכיל **רק** את הנושא המדויק של המייל
   בטקסט חופשי, בלי קישור; (ב) **אימות הצלחה בפועל** — יש לבדוק את תוצאת
   הקריאה בפועל: היא חייבת לכלול `id` תקין של אירוע, בלי שגיאה. **אם הקריאה
   נכשלה, timeout, או שאין `id` בתשובה — לעצור כאן.** אסור לתייג
   `Ami/Event-Created`, אסור להסיר `Ami/Event-Pending`/`Ami/Event-Notify-Queued`,
   ואסור לדווח למשתמש שהאירוע נוצר. ה-thread נשאר בדיוק כמו שהיה (עדיין
   Pending) כדי שינוסה שוב באישור הבא, ויש לציין את הכישלון במפורש בתשובה
   למשתמש. (תוקן 10.07.2026 — ריצה קודמת תייגה thread כ-`Ami/Event-Created`
   בלי שנוצר בפועל אף אירוע ביומן; זה לא אמור לקרות שוב.) (ג) **רק אם (ב)
   אישר הצלחה אמיתית**: תיוג ה-thread ב-`Ami/Event-Created`, והסרת `INBOX`,
   `Ami/Event-Pending`, `Ami/Event-Notify-Queued`. (ד) **רק אם (ג) בוצע**:
   `update_event` נוסף שמוסיף גם שורת קישור חיפוש מבוסס label+subject.
...
### קישור למייל המקור — מבוסס label (עדכון 06.07.2026)

נוסו ארבעה פורמטים קודמים וכולם נכשלו בפועל אצל המשתמש (תמיד הובילו ל-inbox
כללי, לא למייל):

1. קישור ישיר עם `id` של ה-thread.
2. קישור ישיר עם `id` של ההודעה הספציפית מתוך `messages[]`.
3. קישור חיפוש לפי subject בלי `in:anywhere`.
4. קישור חיפוש לפי subject **עם** `in:anywhere`.

**ניסיון חמישי (הנוכחי): שילוב label+subject.** לאחר שה-thread כבר מתויג
`Ami/Event-Created` (כלומר, אחרי שהאירוע כבר נוצר והתיוג כבר בוצע — לא לפני),
בונים קישור חיפוש שמשלב את התווית עם הנושא, מה שמצמצם דרמטית את טווח
החיפוש (רק מיילים שהסוכן כבר טיפל בהם וסימן):

`https://mail.google.com/mail/u/0/#search/label%3AAmi-Event-Created+subject%3A%22{subject מקודד}%22`

(שימו לב: `/` בשם התווית הופך ל-`-` בסינטקס החיפוש של Gmail — `Ami/Event-Created`
→ `Ami-Event-Created`.)

**סדר פעולות מחייב** (חשוב — לא לשנות סדר):
1. יצירת האירוע ביומן, **בלי** קישור בשלב הזה — רק הנושא בטקסט חופשי.
2. תיוג ה-thread ב-`Ami/Event-Created` (וכל שאר תיוגי הארכוב).
3. **רק אז** `update_event` נוסף שמוסיף שורת קישור עם label+subject לתיאור.

**הטקסט עם הנושא המדויק נשאר חובה תמיד**, גם אם מוסיפים את הקישור — לא
מחליפים אותו. בדיקה אוטומטית של הקישור (למשל דרך WebFetch) אינה אפשרית באמת:
Gmail מחזיר 403 ללא session מאומת בדפדפן, כך שאין דרך לאמת מכאן שהקישור אכן
עובד — **הבדיקה האמיתית חייבת להיות ידנית על ידי המשתמש**, בכל אירוע חדש
שנוצר, לפחות עד שיצטבר מספיק ביטחון שהגישה הזו עובדת באופן עקבי.

אם גם הניסיון הזה נכשל, יש לחזור למדיניות "טקסט בלבד, בלי קישור" — אין
לנסות פורמט שישי בלי אישור מהמשתמש.
