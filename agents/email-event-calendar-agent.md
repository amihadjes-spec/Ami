...
3. **יצירת האירוע** — במקרה של אישור, האג'נט יוצר את האירוע ביומן Google Calendar
   ...
   בטקסט חופשי, בלי קישור; (ב) תיוג ה-thread ב-`Ami/Event-Created`; (ג) **רק
   אז** `update_event` נוסף שמוסיף גם שורת קישור חיפוש מבוסס label+subject.
   לאחר היצירה, מסירים את `Ami/Event-Pending` מה-thread.
