...
**קישור למייל המקורי באירוע (מעודכן):**

`https://mail.google.com/mail/u/0/#search/in%3Aanywhere+subject%3A%22{הנושא המדויק של המייל, מקודד ב-URL encoding}%22`

נקודות קריטיות:
1. **חובה `in:anywhere`** — בלי זה, החיפוש מוגבל ל-Inbox בלבד, וכיוון שהמייל
   מועבר לתווית `Ami/Event-Created` ומוסר מה-Inbox (כחלק מהתהליך), הקישור לא
   ימצא אותו ויחזור לברירת מחדל (Inbox ריק/כללי) — זו הייתה הסיבה האמיתית
   לכישלון הקודם, לא בעיית מזהי thread/message.
2. **התאמה מדויקת של הנושא** — יש להעתיק את ה-subject בדיוק כפי שהוא מופיע
   במייל המקורי, כולל רווחים וניקוד, ולקודד אותו נכון ב-URL (רווח→`+`,
   מרכאות→`%22`, נקודתיים→`%3A`, וכו' לפי צורך).
3. **בדיקה בפועל** — לפני שממשיכים להשתמש בפורמט הזה כברירת מחדל קבועה,
   יש לוודא בהרצה הבאה שהקישור אכן מוביל למייל הספציפי ולא ל-Inbox כללי.

(היסטוריה: לפני זה נוסו שני פורמטים של קישור ישיר להודעה — עם `id` של
ה-thread ואז עם `id` של ההודעה הספציפית מתוך `messages[]` — ושניהם נכשלו.
יכול להיות שהסיבה המקורית שיוחסה להם [מזהי permalink לא תואמים] לא הייתה
נכונה, וגם הם היו נכשלים באותה סיבה של scope-חיפוש; לא נבדק לאחור.)
