...
3. **יצירת האירוע** — במקרה של אישור, האג'נט יוצר את האירוע ביומן Google Calendar
   ...
   אם צוינו). **בכל אירוע שנוצר, כברירת מחדל, ה-`description` כולל קישור למייל
   המקור**: `https://mail.google.com/mail/u/0/#all/<threadId>` (ראו הסתייגות
   בסוף הסעיף הזה על מגבלה ידועה בקישור הזה). לאחר היצירה,
   מסירים את `Ami/Event-Pending` מה-thread.
...
**מגבלה ידועה בקישור למייל המקור:** נבדק בפועל (06.07.2026) ששני הפורמטים —
עם `u/0/` וגם בלעדיו — לא תמיד קופצים ישירות להודעה: לפעמים הקישור פותח את ה-
inbox הרגיל במקום את המייל הספציפי. זו כנראה התנהגות טעינה (cold load) של
ממשק ה-web של Gmail, לא בעיה בפורמט הקישור עצמו — ל-Gmail MCP הזמין אין גישה
לכותרת ה-`Message-ID` הגולמית של המייל, כך שאי אפשר לבנות את קישור החיפוש
היציב יותר (`#search/rfc822msgid:...`) שבו משתמשת בדרך כלל גם Google Calendar
עצמה כשהיא יוצרת אירועים מתוך Gmail. בפועל: הקישור עדיין שימושי כ"רמז" (למשל
אפשר לחפש את הנושא/השולח שמופיעים בתיאור האירוע ישירות ב-Gmail), אבל אין
הבטחה שהוא יקפוץ ישירות להודעה בכל דפדפן/מכשיר.
