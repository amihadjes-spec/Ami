...
3. **יצירת האירוע** — במקרה של אישור, האג'נט יוצר את האירוע ביומן Google Calendar
   ...
   אם צוינו). **בכל אירוע שנוצר, כברירת מחדל, ה-`description` כולל את הנושא
   המדויק של המייל בטקסט חופשי, בלי שום קישור** (ראו "קישור למייל המקור —
   ננטש לגמרי" בסוף הסעיף). לאחר היצירה, מסירים את `Ami/Event-Pending`
   מה-thread.
...
### קישור למייל המקור — ננטש לגמרי (06.07.2026)

נוסו ארבעה פורמטים שונים של קישור, כל פעם עם תיאוריה סבירה לכישלון הקודם,
וכולם נכשלו בפועל אצל המשתמש (כל הפעמים הובילו ל-inbox כללי, לא למייל):
...
4. קישור חיפוש לפי subject **עם** `in:anywhere` — גם זה נכשל (נבדק בפועל
   06.07.2026 עם שני מיילי בדיקה שונים).

**מסקנה: אין להוסיף יותר קישור אוטומטי לתיאור האירוע.** ארבעה ניסיונות
עוקבים, כל אחד עם הסבר הגיוני משלו, זה מספיק כדי לקבוע שהבעיה אינה בפורמט
הספציפי של הקישור אלא במשהו יסודי יותר בסביבה (למשל: הדרך שבה ה-web client
של Gmail מטפל ב-URL fragments בטעינה קרה, ללא קשר לתוכן שלהם) שאי אפשר
לאבחן או לתקן עם הכלים הזמינים (`get_thread` לא חושף כותרות גולמיות, ואין
כלי לבדוק רינדור אמיתי בדפדפן מחובר). להמשיך לנחש פורמט חמישי הוא בזבוז זמן.

**המדיניות הקבועה מעכשיו:** תיאור האירוע כולל **רק** טקסט חופשי עם הנושא
המדויק של המייל (למשל: "נוצר אוטומטית על ידי סוכן זיהוי אירועים במייל,
מתוך מייל בנושא: <subject>"), בלי שום קישור. אם המשתמש ירצה למצוא את המייל,
הוא מחפש את הנושא הזה ידנית ב-Gmail. זו הדרך היחידה שהוכחה כאמינה.
